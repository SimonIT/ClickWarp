				...
				if (warps_folder.isDirectory()) {
					...
					if (warps.length == 0) {
						...
						for (int i = 0; i < warps.length; i++) {
							if (sender.hasPermission("clickwarp.warp." + warps[i].getName().replace(".yml", "")) || sender.hasPermission("clickwarp.warp.*")) {
						...
						if (list.size() == 0) {
							...
							for (int i = 0; i < list.size(); i++) {
								...
								if (cfg.getString(warp.getName().replace(".yml", "") + ".name") == null) {
									...
									warp_names += ChatColor.translateAlternateColorCodes('&', cfg.getString(warp.getName().replace(".yml", "") + ".name"));
								}
